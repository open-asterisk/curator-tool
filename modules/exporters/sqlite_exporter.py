...
class SQLiteExporter(aExporter):
    ...
    async def create_table(self, table_name, data_dict):
        """
        Create a table in the SQLite database with the given name, and columns based on the keys and data types of the data_dict.
        """
        try:
            columns = ', '.join([f"{key} {self.get_sqlite_type(value)}" for key, value in data_dict.items()])
            column_names = ', '.join([f'{key}' for key in data_dict.keys()])
            await self.cursor.execute(f'CREATE TABLE IF NOT EXISTS {table_name} ({columns}, UNIQUE({column_names}));')
            await self.conn.commit()
        except Exception as e:
            print(f"[{self.name}] ERROR CREATING TABLE: {e}")
